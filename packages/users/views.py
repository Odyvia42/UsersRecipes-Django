...
def sort_user_list_by_reg_date_desc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('registration_date').desc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
...
def sort_user_list_by_username_asc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('username').asc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
...
def sort_user_list_by_username_desc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('username').desc())
...
def sort_user_list_by_recipes_amount_asc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('num_recipes').asc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
...
def sort_user_list_by_recipes_amount_desc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('num_recipes').desc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
...
def sort_user_list_by_rating_asc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('likes_amount').asc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
...
def sort_user_list_by_rating_desc(request):
    users = User.objects.annotate(num_recipes=Count('recipe', distinct=True)).annotate(likes_amount=Count('recipe__likes')).order_by(F('likes_amount').desc())
    p = Paginator(users, 5)
    page = request.GET.get('page')
    paged_users = p.get_page(page)
